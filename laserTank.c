...
void graph(char** array, int row, int col, int px, int py, char direc, int ex, int ey, char edirec, char** mirror, int max_size);
int playershoot(char** array, int row, int col, int px, int py, char direc, int ex, int ey, char edirec, char laserdirec, char** mirror, int max_size);
...
int main(int argc, char *argv[]) 
{
	...
	else
	{
		...
		else
		{
			...
			mirror = (char **)malloc(col*row*sizeof(int *));

			while(fgets(buf, sizeof(buf), sPtr)!= NULL)
			{
				sscanf(buf, "%d %d %c", &x, &y, &type);
				
				mirror[max_size] = (char *)malloc(3*sizeof(int));
...
/* to prevent player to spawn at the same lane with enemy with the enemy looking at them (prevent auto lose when spawn)*/
	...
	while (exit == 0)
	{
		...
		if enemy direction is looking downward and player is below the enemy and at the same column then it will shoot*/
			char elaserdirec = '|'; 
			exit = enemyshoot(array, row, col, py, px, direction, ex, ey, edirection, elaserdirec, mirror, max_size);
		...
		
		{
			...
			graph(array, row, col, py, px, direction, ex, ey, edirection, mirror, max_size); /* printing the graph */
			...
			{	
				if (direction == 'a' || direction == 'd') /* if the input is left or right then shoot horizontal lazer*/
				{
					...
					exit = playershoot(array, row, col, py, px, direction, ex, ey, edirection, laserdirec, mirror, max_size);
