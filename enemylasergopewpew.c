...
int enemyshoot(char** array, int row, int col, int px, int py, char direc, int ex, int ey, char edirec, char elaserdirec,int** mirror, int max_size, LinkedList* list) 
{
	...
	/*prevents the lazer to go outside border and loop the lazer updated position*/
	{
		...
		else 
		{
			...
			for (i = 0; i < row ; i++) 
			{
				for (k = 0; k < col ; k++) /*a for loop to go through x and y to print out the array*/
				{
					...
					else 
					{
						...
						/*loops into mirror array and checks coordinate and the type of array and print them in array*/
						{

							if (mirror[p][0] == i && mirror[p][1] == k)
							{
								if(mirror[p][2] == 'f') /*checks the type of the mirror if it is f then / */
								{	

									array[i][k] = '/';
								}
								else if (mirror[p][2] == 'b') /*checks the type of the mirror if it is b then \ */
								{
									array[i][k] = '\\';
								}
							}
			...
			LogFrame( list, array, row, col); /*log each frame of the laser*/
